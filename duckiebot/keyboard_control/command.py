...
"""
        ...
        parser.add_argument("hostname", nargs="?", default=None, help="Name of the Duckiebot to control")
        ...
        if not parsed_args.hostname and not parsed_args.exercise:
            dtslogger.error("If you are not runnoing an exercise you must specify the hostname of the robot.")
            raise Exception
...
def run_gui_controller_exercise(hostname, image, duckiebot_ip, network_mode):
    ...
    env = {
        ...
        "ROS_MASTER_URI": "http://%s:11312" % duckiebot_ip,
        "HOSTNAME": hostname,
    }
    volumes = {}
