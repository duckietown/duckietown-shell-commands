...
def get_recipe_project_dir(repository: str, branch: str, location: str) -> str:
    ...
    return os.path.join(
        os.environ.get("DTSHELL_RECIPES", recipes_dir), repository, branch, location.strip("/")
    )
...
def clone_recipe(repository: str, branch: str, location: str) -> bool:
    ...
    try:
        dtslogger.info(f"Downloading recipes in '{recipe_dir}' ...")
        remote_url: str = f"https://github.com/'{repository}'"
        run_cmd(["git", "clone", "-b", branch, "--recurse-submodules", remote_url, recipe_dir])
        return True
...
def update_recipe(repository: str, branch: str, location: str) -> bool:
    ...
    # Check if the recipe is outdated
