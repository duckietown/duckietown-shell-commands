...
INPUT_DISK_IMAGE_URL = (
    f"https://cdimage.ubuntu.com/releases/{UBUNTU_VERSION}/release/"
    f"ubuntu-{UBUNTU_VERSION}-preinstalled-server-arm64+raspi.img.xz"
)
TEMPLATE_FILE_VALIDATOR = {
    "writeable:/data/autoboot/*.yaml": lambda *a, **kwa: validator_autoboot_stack(*a, **kwa),
    "writeable:/data/config/calibrations/*/default.yaml": lambda *a, **kwa: validator_yaml_syntax(*a, **kwa),
}
...
APT_PACKAGES_TO_HOLD = [
    ...
    "initramfs-tools",
    "linux-firmware",
    "linux-headers-5.4.0-1034-raspi",
    "linux-image-5.4.0-1034-raspi",
    "linux-modules-5.4.0-1034-raspi",
    "linux-raspi-headers-5.4.0-1034"
]
...
class DTCommand(DTCommandAbs):
    ...
    def command(shell: DTShell, args):
        ...
        in_file_path = lambda ex: os.path.join(parsed.workdir,
                                               f"{UBUNTU_DISK_IMAGE_NAME}.{ex}".rstrip("."))
        ...
        if "download" in parsed.steps:
            ...
            if not os.path.isfile(in_file_path("xz")):
                ...
                try:
                    ...
                    run_cmd(["rm", "-rf", in_file_path("xz")])
                    raise e
            else:
                dtslogger.info(f"Reusing cached ZIP image file [{in_file_path('xz')}].")
            # unzip (if necessary)
            if not os.path.isfile(in_file_path("img")):
                dtslogger.info("Extracting ZIP image...")
                try:
                    run_cmd(["xz", "-d", "-f", "-k", in_file_path("xz")])
                    run_cmd(["mv", in_file_path(""), in_file_path("img")])
                except KeyboardInterrupt as e:
                    dtslogger.info("Cleaning up...")
        ...
        if "create" in parsed.steps:
            ...
            if os.path.exists(out_file_path("img")) and not parsed.do_continue:
                msg = (
            ...
            try:
                ...
                for partition in partitions:
                    ...
                    if partition not in DISK_IMAGE_PARTITION_TABLE:
                        raise ValueError(
                            f"Partition {partition} not declared in partition table")
                    # check if the corresponding disk device exists
                        ...
                        if partition == ROOT_PARTITION:
                            for stack in list_files(STACKS_DIR, "yaml"):
                                ...
                                destination = os.path.join(
                                    ...
                                    AUTOBOOT_STACKS_DIR.lstrip("/"), stack
                                )
                                ...
                                replace_in_file("{ARCH}", "{ARCH:-%s}" % DEVICE_ARCH,
                                                destination)
                        ...
                        for update in files:
                            ...
                            if validator:
                                ...
                                validator(shell, update["origin"], update["relative"],
                                          arch=DEVICE_ARCH)
                            # create or modify file
                            effect = "MODIFY" if os.path.exists(
                                update["destination"]) else "NEW"
                            dtslogger.info(
                                f"- Updating file ({effect}) [{update['relative']}]")
                            # copy new file
        ...
        if parsed.push:
            if "compress" not in parsed.steps:
                dtslogger.warning(
                    "The step 'compress' was not performed. No artifacts to push.")
                return
        ...
        #         # ubuntu_partition_disk = sd_card.partition_device("HypriotOS")
        #         # if not os.path.exists(ubuntu_partition_disk):
        #         #     raise ValueError(f"Disk device {ubuntu_partition_disk} not found")
        #         # mount `root` partition
        #         sd_card.mount_partition(ROOT_PARTITION)
        #         # from this point on, if anything weird happens, unmount the `root` disk
        #         try:
        ...
        #                 # run_cmd(["sudo", "mount", "-t", "auto", ubuntu_partition_disk, _boot])
        #                 # from this point on, if anything weird happens, unmount the `root` disk
        #                 try:
        #                     # run full-upgrade on the new root
        #                     run_cmd_in_partition(
